...
export const login = async(username, password) => {
    ...
    // const browser =s await puppeteer.launch({headless:false},{defaultViewport: null});
            ...
            const link = await page.evaluate(() => {
              ...
              for (let i of link) {
                ...
                if (i === link[link.length -1]){
                  console.log("I able to open the lastLink to perform the action\n\n\n\n")
                  //  await openLink(i)
                  ...
                  for (let i of last) {
                    console.log(`opening lastLinks: ${i}`)
                    await openLink(i)
...
export const getBal =async(username, password) => {
  // console.clear()
  console.log("Logining IN ")
  ...
  const b = getthebal()
  browser.close()
  return `The current balance of ${username} is ${b} please feel free to check other routes ... `
